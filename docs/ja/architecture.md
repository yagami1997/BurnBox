...
現在の工学的基準線は `4.3 GB / 870 parts` および `11 GB / 2200 parts` までの大容量転送で検証済みです。BurnBox 2.2.1 では owner-claim 認証、legacy upgrade flow、deployment-managed private-entry routing、upload diagnostics、upload-abort cleanup がすでに入っています。次の実装段階は、まず frontend-JS の整理、その後に resumable upload です。
