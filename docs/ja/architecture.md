...
現在の工学的基準線は `4.3 GB / 870 parts` および `11 GB / 2200 parts` までの大容量転送で検証済みです。次の実装段階は、この基準線の上に resumable upload を重ねることです。
...
次に予定している拡張は resume-status layer です。これにより、中断した upload を server-side の durable part truth から継続できるようにします。
