...
- BurnBox 2.1.1 は `4.3 GB / 870 parts` および `11 GB / 2200 parts` までの大容量 multipart transfer で検証済みです
...
BurnBox 2.1.1 では active link は `public_handle` から再構築されます。local browser cache には依存しません。
