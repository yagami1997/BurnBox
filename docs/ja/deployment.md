...
BurnBox 2.2.2 はドメイン分離に加えて、owner account による認証導線を前提にしています。
...
BurnBox 2.2.1 では、運用上の ownership model も変わりました。
...
- `SESSION_SECRET`
- `SHARE_LINK_SECRET`
- 必要なら one-time claim 用の `CLAIM_KEY`

推奨:

- owner session には `SESSION_SECRET`
- 公開 download signature には `SHARE_LINK_SECRET`
- `CLAIM_KEY` は deployment log で claim code を扱いにくい場合の互換手段
